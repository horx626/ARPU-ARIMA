...
def loop_select_six_param():
    p = d = q = range(1, 2)  # ARIMA部分的参数
    P = D = Q = range(0, 2)  # 季节性部分的参数
    s = 7  # 季节性周期长度
    # 创建所有参数组
    ...
    for param in parameters:
        try:
            ...
            # 记录 AIC、BIC、MSE
            aic_value = results_.aic
            bic_value = results_.bic
            forecast = results_.get_forecast(steps=12)
            forecast_values = forecast.predicted_mean
            mse = mean_squared_error(ts[-12:], forecast_values)
...
def SARIMA_fit(P_num,D_num,Q_num,s_num,data,pdq_data):
    model = SARIMAX(data,
                    ...
                    seasonal_order=(P_num, D_num, Q_num, s_num),
                    enforce_stationarity=False,
                    enforce_invertibility=False)
    model_fit = model.fit()
    return model_fit
...
def forecast_data_process(fitData,partDataArr):
    min_value = 0
    forecast = fitData.get_forecast(steps=len(partDataArr[1]))
    forecast_index=pd.date_range(start=partDataArr[0].index[-1]+pd.DateOffset(days=1), periods=len(partDataArr[1]), freq='D')
    fitForecast = forecast.predicted_mean
    forecast_values = np.maximum(fitForecast, min_value)
    forecast_series = pd.Series(forecast_values, index=forecast_index)
    return forecast_series
def min_num(data):
    min_value = 0
    fitData=data.predicted_mean
    data_value = np.maximum(fitData, min_value)
    return data_value
#Ljung_Box检测
#     参数:
